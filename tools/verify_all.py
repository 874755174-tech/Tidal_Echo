...
会依次跑：
 ...
 12. tools/generate_check.py        P2 ⑨ 停止 / 重答 / 多版本（**假身体 + 慢上游**，
                                    专照"停止变成偷偷换模型重跑"这个病）
 13. git diff -- backend/ examples/ channel/  （红线，必须为空）

⚠️ 跑之前先确认 **8080 端口是空的** —— `tools/secaudit.py` 写死用它起测试服务，
   被占（比如那个 `kael-probe` 模型探测器还开着）会让 1/12 整套红，
...
# (标签, [可执行文件, 脚本], 额外 env)
SUITES = [
    ("1/12  访问控制体检", [PY, "secaudit.py"], {}),
    ("2/12  会话数据层 + 兜底", [PY, "sessioncheck.py"], {}),
    ("3/12  兜底四场景 + 鉴权红线", [PY, "sessionfallback_check.py"], {}),
    ("4/12  P0 地基：四张表 + 身份层", [PY, "app_ext_check.py"], {}),
    ("5/12  P1 模型网关：允许列表 + 三格式 + 真 HTTP + 参数下发 + 原始帧诊断 + CoT 透传", [PY, "providers_check.py"], {}),
    ("6/12  web/ 各页面内联 JS 语法", [PY, "jscheck.py"], {}),
    ("7/12  设置页模型/参数前端（jsdom 真跑）", [NODE, "model_ui_check.mjs"],
     {"NODE_PATH": NODE_MODULES}),
    ("8/12  会话归档/删除/改名前端（jsdom 真跑）", [NODE, "session_ui_check.mjs"],
     {"NODE_PATH": NODE_MODULES}),
    ("9/12  房间层：工作间 + MCP 门", [PY, "workshop_check.py"],
     {"RELAY_WORKSHOP_DIR": ""}),
    ("10/12 P2-0 导出 / 快照：一致快照 + 只读 + 密钥不进 URL", [PY, "archive_check.py"], {}),
    ("11/12 P2 ⑧ 上下文管理：注入 / 摘要 / 迁移（从出口倒着验）", [PY, "context_check.py"], {}),
    ("12/12 P2 ⑨ 停止/重答/多版本（假身体 + 慢上游，专照「偷偷换模型重跑」）",
