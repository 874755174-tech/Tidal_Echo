...
### 🔴 上线第一天就被真实数据照出一个 bug：**中转站的"双命名"账单**

记账跑通后（`coverage=1.0`）看 `recent?raw=1`，发现**每一行的 `completion_tokens` 都是 0**，
而 raw 里明明写着 450。根因不在"接错"，在**"以为一份账单只会长成一家的样子"**：
...
中转站（哈基米）自己归一过一遍、又把**原始那一套一起带回来** —— 于是同一份账单里
两套命名并存，而**Anthropic 那一半是坏的**：流式下 `message_start` 先到、那帧的
`output_tokens` 还是上游初值 `0`，站子把两帧并进同一个对象时**先到的 0 把后来的 450 顶掉了**。

- OpenAI 三件套**自己算得平**：`4479 + 450 = 4929` ✓
- Anthropic 那一对**算不平**：`4479 + 0 = 4479 ≠ 4929` ✗

🔴 而原来的 `shape_of()` 是"**认出是哪一家**"的单值判断，看见 `input_tokens` 就先返
`anthropic` → 每次都去读**那套坏的**。最坏的地方是**账面看上去完全正常**：
`total_tokens` 是对的、`coverage` 是 1.0，只有输出那一列静默地全是 0 ——
`usage` 记账设计时最怕的"账本说谎"，第一次就以这种形式来了。

⇒ 判据从"**名字叫什么**"改成"**哪一套自己算得平**"（`_read_family()`）：

| 情况 | 读哪一套 | `shape` |
|---|---|---|
| 只有一家 | 就那一家（四家原逻辑不变） | `openai` / `anthropic` / `deepseek` / `gemini` |
| 双命名、OpenAI 算得平 | OpenAI（**输出 450，不是 0**） | `openai+anthropic` |
| 双命名、Anthropic 算得平 | Anthropic | `anthropic` |
| 双命名、都算不平 | 仍读 OpenAI（它自带 `total_tokens`） | `openai+anthropic` |
| 压根没认出来 | —— **不许声称口径**（这条也一起收口了） | `unknown` |

🔴 双命名时**口径留 `None`（不明）**：两套名字都用了，我们**没有证据**判断它报的
`cache_read`（如果将来有）算不算在 `prompt_tokens` 里（Anthropic 语义并列 / OpenAI 语义子集）。
读数侧对口径不明的行**保守取和** —— 猜一个数 = 拿命中率骗自己。

这份 raw 现在原样躺在 `tools/usage_check.py` 的 G 组里（一个字符没改）当回归夹具：
**同一个坑不踩第二次**。

### 🔴 下一站：缓存**启用** —— 但先答一个问题：**这个站到底报不报缓存**

记账这一缝解决的是"**看得见**"。真实数据一到手，答案就写在那里了：

| 看到了什么 | 说明什么 |
|---|---|
| `usage_source: "anthropic"` | 这个站**背后是 Anthropic**（它自己标的） |
| `cache_read` / `cache_write` 两列**全是 `null`**，`prompt_tokens_details` 里**没有 `cached_tokens`** | 🔴 **这个站一个缓存字段都不报** |
| `cache_hit_rate = null` | 不是"没命中"，是"**我们还看不见**" |

⇒ 所以**先别改请求**。"把每轮都变的那段挪到 prompt 末尾"（→ 前缀稳定、
历史对话吃上缓存）**结构上是对的**，但这一站改了**没法量**，
而没法量的优化等于"觉得应该有用"。

⚠️ 下一站的第一件事是弄清**这个站有没有缓存**：它的控制台/文档里有没有
"缓存命中"这一档计费。判据是硬的 —— **有缓存能力的站，账单里就会有缓存字段**
（OpenAI 系的 `cached_tokens`、Anthropic 系的 `cache_read_input_tokens`），
而这一站现在**一个都没有**。两种可能，指向两条路：
